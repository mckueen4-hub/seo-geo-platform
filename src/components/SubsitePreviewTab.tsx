...
import { Globe, CheckCircle, ExternalLink, Copy, Code, Sparkles, BookOpen, Clock } from 'lucide-react';
...
export const SubsitePreviewTab: React.FC<SubsitePreviewTabProps> = ({
  ...
  const langCode = activeLangTab === 'TC' ? 'hk' : activeLangTab === 'SC' ? 'cn' : 'en';

  const langArticles = (activeStore?.articles || []).filter(a => a.audience === langCode || (activeLangTab === 'TC' && a.audience === 'tw'));
  const latestArticle = langArticles[0] || activeStore?.articles[0];
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="glass-panel" style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#9ca3af', marginTop: '4px' }}>
            點擊下方頁籤可切換查看 <strong>繁體中文 (港/台)</strong>、<strong>簡體中文 (內地小紅書)</strong> 與 <strong>Native English (歐美Expat)</strong> 獨立文章歷程與存檔！
          </p>
        </div>
      ...
      <div className="glass-panel" style={{ padding: '0', overflow: 'hidden', border: '1px solid #374151' }}>
        ...
        <div style={{ padding: '32px', background: '#0b0f19', color: '#f3f4f6' }}>
            ...
            <div style={{ background: '#111827', padding: '24px', borderRadius: '12px', border: '1px solid #1f293d', marginBottom: '28px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <span style={{ fontSize: '12px', background: activeLangTab === 'SC' ? 'rgba(244, 63, 94, 0.2)' : activeLangTab === 'EN' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(59, 130, 246, 0.2)', color: activeLangTab === 'SC' ? '#fb7185' : activeLangTab === 'EN' ? '#34d399' : '#60a5fa', padding: '4px 10px', borderRadius: '6px', fontWeight: '700' }}>
                  📝 🔥 最新每日 AI 文章 ({latestArticle.topic}) - [{activeLangTab === 'TC' ? '繁體中文' : activeLangTab === 'SC' ? '簡體中文' : 'Native English'}]
                </span>
                <span style={{ fontSize: '12px', color: '#9ca3af', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <Clock size={13} /> 發佈時間: {latestArticle.createdAt}
                </span>
              </div>
              ...
              <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid #1f293d', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {latestArticle.aiSourcesCited?.map((src, idx) => (
          ...
          {/* Historical Daily Articles Archive Flow */}
          {historyArticles.length > 0 && (
            <div style={{ background: '#0e1422', padding: '24px', borderRadius: '12px', border: '1px solid #1f293d', marginBottom: '28px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ fontSize: '16px', fontWeight: '800', color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  📚 歷史每日 AI 文章動態發佈存檔 (累積共 {langArticles.length} 篇)
                </h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {historyArticles.map((art, idx) => (
                  <div key={art.id || idx} style={{ background: '#111827', padding: '18px', borderRadius: '10px', border: '1px solid #1f293d' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      <span style={{ fontSize: '11px', background: 'rgba(251, 191, 36, 0.15)', color: '#fbbf24', padding: '2px 8px', borderRadius: '4px', fontWeight: '700' }}>
                        歷史存檔：{art.topic}
                      </span>
                      <span style={{ fontSize: '11px', color: '#9ca3af' }}>{art.createdAt}</span>
                    </div>

                    <h4 style={{ fontSize: '16px', fontWeight: '700', color: '#f3f4f6', marginBottom: '8px' }}>
                      {art.title}
                    </h4>
